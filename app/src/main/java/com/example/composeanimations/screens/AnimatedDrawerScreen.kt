...
@Composable
fun AnimatedDrawerScreen(modifier: Modifier = Modifier) {
    Box {
        ...
        val draggableState = rememberDraggableState { dragAmmount ->
        ...
        CustomDrawer(selectedScreen = screenState, onScreenSelected = { screen ->
            screenState = screen
        })
        ...
        MainContent(toggleDrawer = { toggleDrawerState() },
            /**
            ...
            modifier = modifier
                .graphicsLayer {
                     ...
                     * */
                    val cornerShapeInterpolator =
                        lerp(0.dp.toPx(), 32.dp.toPx(), transXToPositive / drawerWidth)
                ...
                .draggable(draggableState, Orientation.Horizontal, onDragStopped = {
                    ...
                    coroutineScope.launch {
                        /**
                         * Check If DecayX is greater than the half of drawer width.
                         * If greater than drwer width then we will open the drawer else move back the drawer to closed state.
                         * */
                        val targetX = if (decayX > drawerWidth * 0.5) drawerWidth else 0f
                        if (targetX == drawerWidth) {
                            translationX.animateTo(
                                targetX, initialVelocity = velocity, animationSpec = spring(
                            ...
                            translationX.animateTo(
                                0f, initialVelocity = velocity, animationSpec = spring(
                                    dampingRatio = DampingRatioLowBouncy,
                                    stiffness = StiffnessMediumLow,
                                )
                            )
...
@Composable
fun AnimatedDrawerUsingAnchorScreen(modifier: Modifier = Modifier) {
    Box {
        var drawerState by remember {
            mutableStateOf(DrawerState.Closed)
        }
        ...
        val draggableState = remember {
            AnchoredDraggableState(initialValue = DrawerState.Closed,
                anchors = anchors,
        ...
        fun toggleDrawerState() {
            coroutineScope.launch {
                Log.d("First Screen", "toggleDrawer")
                drawerState = if (drawerState == DrawerState.Open) {
                    draggableState.animateTo(DrawerState.Closed)
                    DrawerState.Closed
                } else {
                    draggableState.animateTo(DrawerState.Open)
                    DrawerState.Open
                }
            }
        }
        ...
        MainContent(toggleDrawer = { toggleDrawerState() },
            ...
            modifier = modifier
                .graphicsLayer {
                     ...
                     * lerp = linearly interpolate between two values 1 - 0.8
                     * */
                    val scale = lerp(1f, 0.8f, draggableState.requireOffset() / drawerWidth)
                    this.scaleX = scale
                    this.scaleY = scale
                    /**
                     * As we have added DampingRatioMediumBouncy to the spring animation,
                     ...
                     * sure it doesn't causes an crash as Corner radius can never be in negative.
                     * */
                    val transXToPositive = if (draggableState.requireOffset() < 0f) 0f else draggableState.requireOffset()
...
@Composable
fun MainContent(
    toggleDrawer: () -> Unit, modifier: Modifier
) {
...
private enum class Screen(val text: String, val icon: ImageVector) {
    Home("Home", Icons.Default.Home), SleepDetails(
        "Account",
        Icons.Default.AccountCircle
    ),
    Leaderboard("Calender", Icons.Default.DateRange), Settings("Settings", Icons.Default.Settings),
}
